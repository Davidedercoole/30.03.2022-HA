...
class pewpew(pygame.sprite.Sprite):
    def __init__(self, pos_x, pos_y, player_speed_x, player_speed_y, winkel) -> None:
        super().__init__()
        self.height = 20
        self.width = 20 
        self.image_original = pygame.image.load(os.path.join(Settings.path_image, "pewpew.png"))
        self.size = Settings.pewpew_size
    ...
    def change(self):
        self.winkelc = 0
        self.winkelc = (math.pi/180) * self.winkel
    ...
    def löschen(self):
        if self.rect.top + self.speed_y > Settings.window_height:
            self.kill()
        if self.rect.bottom + self.speed_y < 0:
            self.kill()
        if self.rect.right + self.speed_x < 0:
            self.kill()
        if self.rect.left + self.speed_x > Settings.window_width:
            self.kill()
    ...
    def geschwindigkeit(self):
        self.change()
        self.speed_y = self.speed_y - math.cos(self.winkelc)
        self.speed_x = self.speed_x - math.sin(self.winkelc)
...
class Game(object):
    def __init__(self) -> None:
        ...
        self.pewpews = pygame.sprite.Group()
    ...
    def draw(self): #erstellt die einzeln objekte auf dem bildschirm
            ...
            self.pewpews.draw(self.screen)
            pygame.display.flip()


    def pow(self):
        if len(self.pewpews.sprites()) <= 10:
            self.pewpews.add(pewpew(self.player.rect.centerx, self.player.rect.centery, self.player.speed_x, self.player.speed_y, self.player.angle))
